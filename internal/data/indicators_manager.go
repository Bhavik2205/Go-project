...
import (
	...
	// Add time import for time.Time comparison if needed
	"github.com/Bhavik2205/ML-Bot/internal/db"
	"github.com/Bhavik2205/ML-Bot/internal/indicators"
	"github.com/Bhavik2205/ML-Bot/internal/utils"
	"github.com/gorilla/websocket"
...
// This function is now called by worker goroutines.
func (im *IndicatorManager) handleOutput(indicator indicators.IndicatorResult) {
	// Determine the conflict columns based on the indicator type
	var conflictColumns []clause.Column
	indicatorName := indicator.GetIndicatorName()

	// All indicators have these primary keys
	...
	// Add specific primary key columns based on the indicator type
	...
	case "OBV", "VWAP":
		// These indicators only have InstrumentToken, Interval, Timestamp as primary keys
		// No additional columns needed here beyond the common ones already added
	default:
		// This case should ideally not be hit if all indicators are covered.
		// Log a warning or error if an unknown indicator name appears.
	...
	err = im.dbClient.DB.Clauses(clause.OnConflict{
		...
		DoUpdates: clause.AssignmentColumns(db.GetUpdatableIndicatorColumns(indicatorName)), // Use the helper
		...
		if ind.IsEnabled(im.indicatorsCfg) {
			...
			go func(indicatorType indicators.Indicator) {
				...
				case []indicators.ADX:
					if len(v) > 0 {
						latest := v[len(v)-1]
						if !math.IsNaN(latest.ADXValue) { // ADX can be NaN for initial periods
							im.processedIndicatorCh <- latest
						} else {
							zap.L().Warn(fmt.Sprintf("Latest %s value is NaN or no results", indicatorType.GetName()), zap.Uint32("token", token), zap.String("interval", interval))
						}
